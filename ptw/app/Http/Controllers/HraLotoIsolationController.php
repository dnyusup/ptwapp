...
class HraLotoIsolationController extends Controller
{
    ...
    {
        try {
            \Log::info('[HRA LOTO] handlePhotoUpload started', [
            ...
            if ($request->hasFile('work_area_photo')) {
                ...
                \Log::info('[HRA LOTO] File upload successful', ['path' => $path]);
                return $path;
            }
            ...
            if ($request->filled('work_area_photo_data')) {
                ...
                \Log::info('[HRA LOTO] Processing base64 data', [
                ...
                if ($imageData === false) {
                    \Log::error('[HRA LOTO] Base64 decode failed');
                    return null;
                }
                
                \Log::info('[HRA LOTO] Base64 decoded', ['decoded_length' => strlen($imageData)]);
                
                // Ensure directory exists
                $directory = storage_path('app/public/hra_photos');
                if (!file_exists($directory)) {
                    mkdir($directory, 0755, true);
                    \Log::info('[HRA LOTO] Created directory', ['path' => $directory]);
                }
                ...
                \Log::info('[HRA LOTO] Base64 save result', [
                    'filename' => $filename,
                    'path' => $path,
                    'result' => $result,
                ...
                if (!$result) {
                    \Log::error('[HRA LOTO] Storage::put returned false');
                    return null;
                }
            ...
            \Log::error('[HRA LOTO] Photo upload exception', [
                'error' => $e->getMessage(),
        ...
        \Log::warning('[HRA LOTO] No photo data provided');
        return null;
    }
